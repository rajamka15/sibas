...
<div id="accordion">
  ...
  foreach($periode_bansos_list as $data) {
   ...
   <input type="hidden" name="kuota_<?=$no?>" id="kuota_<?=$no?>" value="<?=$data['kuota'];?>">
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      $.each(res.data_hasil,function(keys,values){
        ...
        table_content +=`
        <div class="tab-pane fade active show" id="tabs-`+key+`-`+keys+`" role="tabpanel" aria-labelledby="custom-tabs-three-home-`+key+`-`+keys+`">
     ...
     var table_penduduk =`
     <div class="tab-pane fade" id="tabs-three-home`+key+`" role="tabpanel" aria-labelledby="custom-tabs-three-home`+key+`">
    ...
    var limit = parseInt($('#kuota_'+key).val())
 var tbody =$('#tbody_2_2');
